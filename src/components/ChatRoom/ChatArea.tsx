...
import { GetAnswer, SendNoInterest, SetSemester } from "@/server";
...
const ChatArea: React.FC<Props> = ({ isHistory }: Props) => {
  ...
  const [price, setPrice] = useState(initialMarketingPrice);
  const [hasAlteredPrice, setHasAlteredPrice] = useState<boolean>(false);
  const [dialogMessage, setDialogMessage] = useState<string>("");
  const [dialogTitle, setDialogTitle] = useState<string>("");
  const [key, setKey] = useState<number>(0);
  const [showMarketingModalNoInterest, setShowMarketingModalNoInterest] =
    useState(false);
  ...
  useEffect(() => {
    setPrice(initialMarketingPrice);
  }, [initialMarketingPrice]);
  ...
  const handleConfirmSemesterModal = async () => {
    if (semester.length > 0) {
      dispatch(userActions.setShowSemesterModal(false));
      const token = await getAccessToken(dispatch, selector);
      await SetSemester(token, semester);
      dispatch(userActions.setSemester(semester));
      setSemester("");
    } else {
      alert(
        "ST"
          ? i18n(translation, "userInfo", "selectSemesterST")
          : occupation == "AUS"
          ? i18n(translation, "userInfo", "selectSemesterAUS")
          : "Bitte gib Info ein"
      );
    }
  };
        ...
        try {
          ...
          if (res.success === 0 && retry > 0) {
            ...
            if (res.answer === "CANNOT_ANSWER") {
              ...
              if (res.marketingMsg && res.marketingMsg.length > 0) {
                dispatch(
                  userActions.setInitialPriceMarketing(
                    res.marketingMsg[0]?.initialPrice
                  )
                );
                setTimeout(function () {
                  dispatch(
          ...
          if (retry > 0) {
            ...
            setTimeout(() => logout(router, dispatch), 3000);
            // dispatch(
            //   pharmazingActions.addToMessages(
            //     "system",
            //     i18n(translation, "question", "unableProcess") + ".",
            ...
            //   )
            // );
            // setTimeout(() => setShowSessionExpired(true), 1000);
            // setTimeout(() => logout(router, dispatch), 6000);
            return;
          }
        }
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!isHistory) {
      ...
      setTimeout(() => {
        if (scrollDivRef.current) {
          scrollDivRef.current.scrollTop =
            scrollDivRef.current.scrollHeight * 2;
        }
      }, 25);
    }
  };
  ...
  return (
    <div className="flex-1 bg-[#f5f5f5] relative">
      <Modal
        ...
        description={
          occupation == "ST"
            ? i18n(translation, "userInfo", "semesterST_title")
            : occupation == "AUS"
            ? i18n(translation, "userInfo", "semesterAUS_title")
            : i18n(translation, "userInfo", "semesterST_title")
        }
        inputValue={semester}
        onChangeInput={(text) => setSemester(text)}
        submitButtonText={i18n(translation, "general", "confirm")}
        submitHandler={handleConfirmSemesterModal}
        closeButtonText={
          occupation === "BE"
            ? i18n(translation, "general", "cancel")
            : undefined
        }
        closeHandler={async () => {
          if (occupation === "BE") {
            dispatch(userActions.setShowSemesterModal(false));
            const token = await getAccessToken(dispatch, selector);
            await SetSemester(token, "CANCEL");
          }
        }}
      />
      <Modal
        theme="light"
        showModal={showMarketingModalNoInterest}
        title={i18n(translation, "question", "marketingNoInterestModalTitle")}
        description={i18n(
          translation,
          "question",
          "marketingNoInterestModalDescription"
        )}
        submitButtonText={i18n(translation, "general", "yes")}
        submitHandler={async () => {
          setShowMarketingModalNoInterest(false);
          const token = await getAccessToken(dispatch, selector);
          await SendNoInterest(token, marketingId);
        }}
        closeButtonText={i18n(translation, "general", "cancel")}
        closeHandler={() => setShowMarketingModalNoInterest(false)}
      />
      <Modal
        theme="light"
      ...
      <Modal
        ...
        title={"Deine Session ist abgelaufen"}
        submitButtonText="Ok"
        submitHandler={() => logout(router, dispatch)}
      />
      {/* Chat Messages */}
      ...
      >
        {isHistory && loadingHistory ? (
          ...
          <>
            ...
            {userMessages.length > 0 &&
              // userMessages.slice(4).map((message: any, idx: number) => {
                ...
                return (
                  ...
                  >
                    <MessageBox
                      ...
                      price={price}
                      setPrice={setPrice}
                      hasAlteredPrice={hasAlteredPrice}
