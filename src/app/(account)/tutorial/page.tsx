...
const Tutorial: NextPage<Props> = (props: Props) => {
  ...
  const goNext = () => {
    if (currentIdx === getMaxIdx(studies)) {
      ...
      setTimeout(() => router.replace("/Frage"), 100);
    } else {
  ...
  const checkImgSrc = (url: string) => {
    return /\/WEB\//.test(url);
  };

  return (
    <div
      className={`${(currentIdx === 0 || currentIdx === getMaxIdx(studies)) && "tutorial-bg"
        }
    ...
    >
      ...
      <div
        className={`flex flex-col items-center justify-start text-white ${
          ...
          checkImgSrc(getImg(currentIdx, studies).uri)
      ...
      >
        ...
        {currentIdx > 0 && currentIdx < getMaxIdx(studies) && (
          <>
            <h1
              className={`font-fontBold font-bold mt-8 mb-2 text-center ${
                currentIdx != getVideo3DIdx(studies) &&
                currentIdx != getVideoUpVoteIdx(studies) &&
                !(
                  currentIdx === 6 &&
            ...
            {text.description && (
              <p
                className={`font-fontBold font-bold text-center mb-2 ${
                  currentIdx != getVideo3DIdx(studies) &&
                  currentIdx != getVideoUpVoteIdx(studies) &&
                  !(
                    currentIdx === 6 &&
            ...
            {currentIdx === getVideo3DIdx(studies) && (
              <div className="bg-secondary px-1 py-[3px] mb-2">
                <p className="font-fontBold text-lg font-bold text-center leading-4">
                  {text.para1}
                </p>
              </div>
            )}
            ...
            {currentIdx === getVideo3DIdx(studies) && (
                <div className="w-full h-[150%] bg-transparent">
                  <video
                    width={2866}
                    height={1618}
                    autoPlay
                  ...
                  >
                    <source src="/assets/videos/3D.mp4" />
            ...
            {currentIdx === getVideoUpVoteIdx(studies) && (
              ...
              <video
                ...
                className="rounded-xl m-0 p-0 border-none outline-none"
              >
                <source src="/assets/videos/Upvote.mp4" />
                Your browser does not support the video tag.
              </video>
            </div>
            )}
