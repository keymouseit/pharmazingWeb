...
const Register: NextPage<Props> = (props: Props) => {
  ...
  return (
    <div className="flex flex-col justify-center bg-white items-center w-screen h-screen">
      ...
      <div className="flex flex-col w-[90%] md:w-[50%] lg:w-[30%] items-start text-black">
        <h1 className="font-fontBold text-4xl font-extrabold">
          {text.myNumber}
        </h1>
        ...
        <form className="mt-10 w-full" onSubmit={handleSubmit}>
          <input
            ...
            className="bg-transparent font-semibold border-secondary border-b caret-secondary px-1 pb-2 w-full"
            autoFocus
          />
          <br />
          <button
            type="submit"
            disabled={isSubmitting}
            className="mt-6 px-4 py-3 w-full rounded-custom bg-black hover:bg-opacity-70 disabled:bg-opacity-60 text-white font-fontBold font-bold text-lg"
          >
            <span>
              {isSubmitting ? (
