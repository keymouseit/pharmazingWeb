...
const ResetUserPassword: NextPage<Props> = (props: Props) => {
  ...
  const actionUponCompletion = async (success: boolean, email: string) => {
    if (success) {
      const res: {
        success: boolean;
        error_code?: any;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!isValidPassword(password)) {
      ...
      const res: { success: boolean; email: string } = await ResetPassword(
        telephone.replace(/\s+/g, ''),
        code,
        password
      );
      await actionUponCompletion(res.success, res.email);
    }
  };
