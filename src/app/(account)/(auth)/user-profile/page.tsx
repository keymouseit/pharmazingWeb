...
const ProfileSetup: NextPage<Props> = (props: Props) => {
  ...
  const [text, setText] = useState<{
    ...
    nameBeRegion: string;
    continue: string;
    semesterST_title: string;
    semesterAUS_title: string;
    semesterST_placeholder: string;
    semesterAUS_placeholder: string;
    selectSemesterST: string;
    selectSemesterAUS: string;
  }>({
    validCountry: "",
    fillInCountry: "",
    validOcupation: "",
    ...
    semesterST_title: "",
    semesterAUS_title: "",
    semesterST_placeholder: "",
    semesterAUS_placeholder: "",
    selectSemesterST: "",
    selectSemesterAUS: ""
  });
  ...
  useLayoutEffect(() => {
    setText({
      ...
      semesterST_title: i18n(translation, "userInfo", "semesterST_title"),
      semesterAUS_title: i18n(translation, "userInfo", "semesterAUS_title"),
      semesterST_placeholder: i18n(translation, "userInfo", "semesterST_placeholder"),
      semesterAUS_placeholder: i18n(translation, "userInfo", "semesterAUS_placeholder"),
      selectSemesterST: i18n(translation, "userInfo", "selectSemesterST"),
      selectSemesterAUS: i18n(translation, "userInfo", "selectSemesterAUS")
    });
  }, [translation]);
  ...
  const onChangeSemester = (event: React.ChangeEvent<HTMLInputElement>) => {
    const inputText: string = event.target.value;
    setSemester(inputText);
  };
  ...
  const isValidSemester = () => {
    ...
    if(selectedOccupation.value == 'BE') {
        return true;
    }
    return false;
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!isValidCountry()) {
      ...
      if(selectedOccupation.value == 'ST') {
          setDialogTitle(text.selectSemesterST);
      } else {
          setDialogTitle(text.selectSemesterAUS);
      }
      setShowDialog(true)
    } else {
      sessionStorage.setItem(
        ...
        JSON.stringify({
          ...
          semester: (selectedOccupation.value=='BE' && semester.length==0) ? 'NOT_CHOSEN' : semester
        })
      );
      ...
      <div className="flex flex-col w-[90%] md:w-[60%] lg:w-[30%] items-start text-black">
        <form onSubmit={handleSubmit} className="w-full">
          ...
          {(country.length > 0 || selectedCountry.value != "NONE") &&
            ...
            { (country.length>0 || selectedCountry.value != "NONE") && (selectedStudies.value != "NONE") && (selectedUniversity.value != "NONE" || university.length>0) && selectedOccupation.value != 'BE' &&
            (
              <div className="mb-2 w-full">
                <p className="font-fontBold text-2xl font-bold mb-2">
                  {selectedOccupation.value == "ST" && text.semesterST_title}
                  {selectedOccupation.value == "AUS" && text.semesterAUS_title}
                </p>
                <input
                  type="text"
                  placeholder={
                    selectedOccupation.value == "ST"
                        ? text.semesterST_placeholder
                        : text.semesterAUS_placeholder
                  }
                  value={semester}
